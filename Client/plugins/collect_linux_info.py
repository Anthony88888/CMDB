...
def get_disk_info():
    ...
    :return:
    """
    raw_data = subprocess.Popen("sudo dmidecode -s system-product-name", stdout=subprocess.PIPE, shell=True)
    raw_data = raw_data.stdout.read().decode()
    data_list = raw_data.strip()
    model = data_list

    sn_data = subprocess.Popen("sudo dmidecode -s system-serial-number", stdout=subprocess.PIPE, shell=True)
    sn_data = sn_data.stdout.read().decode()
    sn_list = sn_data.strip()
    sn = sn_list
