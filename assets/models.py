...
class CPU(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)  # 设备上的cpu肯定都是一样的,所以不需要建立多个cpu数据.
...
class Disk(models.Model):
    ...
    model = models.CharField('硬盘型号', max_length=128, blank=True, null=True)
    manufacturer = models.CharField('硬盘制造商', max_length=128, blank=True, null=True)
    capacity = models.FloatField('硬盘容量(GB)', blank=True, null=True)
