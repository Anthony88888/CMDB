...
def dashboard(request):
    ...
    up_rate = round(int(upline/total*100))
    o_rate = round(int(offline / total * 100))
    un_rate = round(int(unknown / total * 100))
    bd_rate = round(int(breakdown / total * 100))
    bu_rate = round(int(backup / total * 100))
...
def report(request):
    if request.method == 'POST':
        ...
        if sn:
            asset_obj = models.Asset.objects.filter(sn=sn)  # [obj,obj...]
            if asset_obj:
